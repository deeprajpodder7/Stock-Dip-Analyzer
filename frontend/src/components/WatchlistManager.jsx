 ...
 */
export const WatchlistManager = ({ tickers, onChanged, maxCustom }) => {
  const [value, setValue] = useState("");
  const [busy, setBusy] = useState(false);

  const customCount = tickers.filter((t) => !t.is_default).length;
  ...
  return (
    <div data-testid="watchlist-manager" className="flex flex-col gap-4">
      <form onSubmit={handleAdd} className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground ml-2" data-testid="watchlist-count">
