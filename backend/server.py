...
async def investment_plan(budget: int = 5000):
    ...
    try:
        ...
        try:
            ...
            print("Timeout in investment plan")
            results = []

        candidates = []
        for ticker, res in zip(MARKET_UNIVERSE, results):
            if isinstance(res, Exception) or not res or res.get("error"):
                continue
        ...
        if not picks:
            ...
            fallback_analysis = next(
                (r for r, t in zip(results, MARKET_UNIVERSE)
                 if t == fallback_ticker and isinstance(r, dict) and not r.get("error")),
                None
            )
            ...
            reason = f"No meaningful dip found. Allocating full ₹{budget} to NIFTYBEES."
            ...
            if len(picks) == 1:
                reason = f"Only 1 strong opportunity found."
            else:
                reason = "Top 2 opportunities selected."
